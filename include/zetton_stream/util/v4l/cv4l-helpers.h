...
class cv4l_fd : v4l_fd {
 public:
  cv4l_fd() { v4l_fd_init(this); }
  cv4l_fd(cv4l_fd *fd) { *this = *fd; }
  cv4l_fd &operator=(const cv4l_fd &_fd) {
    memcpy(this, &_fd, sizeof(_fd));
    if (_fd.fd >= 0) fd = dup(_fd.fd);
    return *this;
  }
  ...
  int open(const char *devname, bool non_blocking = false) {
    return v4l_open(this, devname, non_blocking);
  }
  int s_fd(int fd, const char *devname, bool direct) {
    return v4l_s_fd(this, fd, devname, direct);
  }
  int subdev_open(const char *devname, bool non_blocking = false) {
    return v4l_subdev_open(this, devname, non_blocking);
  }
  int subdev_s_fd(int fd, const char *devname) {
    return v4l_subdev_s_fd(this, fd, devname);
  }
  int media_open(const char *devname, bool non_blocking = false) {
    return v4l_media_open(this, devname, non_blocking);
  }
  int media_s_fd(int fd, const char *devname) {
    return v4l_media_s_fd(this, fd, devname);
  }
  int close() { return v4l_close(this); }
  int reopen(bool non_blocking = false) {
    return v4l_reopen(this, non_blocking);
  }
  ssize_t read(void *buffer, size_t n) { return v4l_read(this, buffer, n); }
  ssize_t write(const void *buffer, size_t n) {
    return v4l_write(this, buffer, n);
  }
  void *mmap(size_t length, off_t offset) {
    return v4l_mmap(this, length, offset);
  }
  int munmap(void *start, size_t length) {
    return v4l_munmap(this, start, length);
  }
  ...
  int querycap(v4l2_capability &cap, bool force = false) {
    if (force) return cv4l_ioctl(VIDIOC_QUERYCAP, &cap);
    cap = this->cap;
    return 0;
  }
  ...
  int query_ext_ctrl(v4l2_query_ext_ctrl &qec, bool next_ctrl = false,
                     bool next_compound = false) {
    return v4l_query_ext_ctrl(this, &qec, next_ctrl, next_compound);
  }

  int g_ctrl(v4l2_control &ctrl) { return cv4l_ioctl(VIDIOC_G_CTRL, &ctrl); }

  int s_ctrl(v4l2_control &ctrl) { return cv4l_ioctl(VIDIOC_S_CTRL, &ctrl); }

  int g_ext_ctrls(v4l2_ext_controls &ec) { return v4l_g_ext_ctrls(this, &ec); }

  int try_ext_ctrls(v4l2_ext_controls &ec) {
    return v4l_try_ext_ctrls(this, &ec);
  }

  int s_ext_ctrls(v4l2_ext_controls &ec) { return v4l_s_ext_ctrls(this, &ec); }
  ...
  int g_selection(v4l2_selection &sel) { return v4l_g_selection(this, &sel); }

  int s_selection(v4l2_selection &sel) { return v4l_s_selection(this, &sel); }
  ...
  int g_tuner(v4l2_tuner &tuner, unsigned index = 0) {
    ...
    if (ret == 0 && tuner.rangehigh > INT_MAX) tuner.rangehigh = INT_MAX;
    return ret;
  }

  int s_tuner(v4l2_tuner &tuner) { return cv4l_ioctl(VIDIOC_S_TUNER, &tuner); }
  ...
  int dqevent(v4l2_event &ev) { return cv4l_ioctl(VIDIOC_DQEVENT, &ev); }

  int g_input(__u32 &input) { return cv4l_ioctl(VIDIOC_G_INPUT, &input); }

  int s_input(__u32 input) { return cv4l_ioctl(VIDIOC_S_INPUT, &input); }

  int g_output(__u32 &output) { return cv4l_ioctl(VIDIOC_G_OUTPUT, &output); }

  int s_output(__u32 output) { return cv4l_ioctl(VIDIOC_S_OUTPUT, &output); }
  ...
  int g_std(v4l2_std_id &std) { return cv4l_ioctl(VIDIOC_G_STD, &std); }

  int s_std(v4l2_std_id std) { return cv4l_ioctl(VIDIOC_S_STD, &std); }

  int query_std(v4l2_std_id &std) { return cv4l_ioctl(VIDIOC_QUERYSTD, &std); }
  ...
  int g_priority(__u32 &prio) { return cv4l_ioctl(VIDIOC_G_PRIORITY, &prio); }
  ...
  int querybuf(v4l_buffer &buf) {
    return v4l_buffer_querybuf(this, &buf, v4l_buffer_g_index(&buf));
  }
  ...
  int dqbuf(v4l_buffer &buf) { return v4l_buffer_dqbuf(this, &buf); }

  int qbuf(v4l_buffer &buf) { return v4l_buffer_qbuf(this, &buf); }
  ...
  int enum_freq_bands(v4l2_frequency_band &band, unsigned tuner,
  ...
  int enum_dv_timings(v4l2_enum_dv_timings &timings, bool init = false,
                      int index = 0) {
    if (init) {
  ...
  int enum_framesizes(v4l2_frmsizeenum &frm, __u32 init_pixfmt = 0,
                      int index = 0) {
    if (init_pixfmt) {
  ...
  int enum_frameintervals(v4l2_frmivalenum &frm, __u32 init_pixfmt = 0,
                          __u32 w = 0, __u32 h = 0, int index = 0) {
    if (init_pixfmt) {
      memset(&frm, 0, sizeof(frm));
      frm.pixel_format = init_pixfmt;
      frm.width = w;
  ...
  int log_status() { return cv4l_ioctl(VIDIOC_LOG_STATUS, NULL); }
  ...
  v4l2_fract g_pixel_aspect(unsigned &width, unsigned &height,
                            unsigned type = 0) {
    v4l2_cropcap ratio;
    ...
    height = 480;
    if (!g_std(std)) {
      if (std & V4L2_STD_525_60) return hz60;
    ...
    if (!g_dv_timings(timings)) {
      ...
      if (width == 720 && height == 480) return hz60;
...
class cv4l_fmt : public v4l2_format {
 public:
  cv4l_fmt(unsigned _type = 0) { v4l_format_init(this, _type); }
  cv4l_fmt(const v4l2_format &_fmt) { memcpy(this, &_fmt, sizeof(_fmt)); }
  ...
  void s_xfer_func(unsigned xfer_func) {
    v4l_format_s_xfer_func(this, xfer_func);
  }
  unsigned g_ycbcr_enc() const { return v4l_format_g_ycbcr_enc(this); }
  unsigned g_hsv_enc() const { return v4l_format_g_hsv_enc(this); }
  void s_ycbcr_enc(unsigned ycbcr_enc) {
    v4l_format_s_ycbcr_enc(this, ycbcr_enc);
  }
  unsigned g_quantization() const { return v4l_format_g_quantization(this); }
  void s_quantization(unsigned quantization) {
    v4l_format_s_quantization(this, quantization);
  }
  ...
  __u32 g_bytesperline(unsigned plane = 0) const {
    return v4l_format_g_bytesperline(this, plane);
  }
  void s_bytesperline(__u32 bytesperline, unsigned plane = 0) {
    v4l_format_s_bytesperline(this, plane, bytesperline);
  }
  __u32 g_sizeimage(unsigned plane = 0) const {
    return v4l_format_g_sizeimage(this, plane);
  }
  void s_sizeimage(__u32 sizeimage, unsigned plane = 0) {
  ...
  unsigned g_first_field(v4l2_std_id std) const {
...
class cv4l_queue : v4l_queue {
 ...
 public:
  ...
  unsigned g_length(unsigned plane) const {
    return v4l_queue_g_length(this, plane);
  }
  unsigned g_mem_offset(unsigned index, unsigned plane) const {
    return v4l_queue_g_mem_offset(this, index, plane);
  }
  void *g_mmapping(unsigned index, unsigned plane) const {
    return v4l_queue_g_mmapping(this, index, plane);
  }
  void s_mmapping(unsigned index, unsigned plane, void *m) {
    v4l_queue_s_mmapping(this, index, plane, m);
  }
  void *g_userptr(unsigned index, unsigned plane) const {
    return v4l_queue_g_userptr(this, index, plane);
  }
  void s_userptr(unsigned index, unsigned plane, void *m) {
    v4l_queue_s_userptr(this, index, plane, m);
  }
  void *g_dataptr(unsigned index, unsigned plane) const {
    return v4l_queue_g_dataptr(this, index, plane);
  }
  int g_fd(unsigned index, unsigned plane) const {
    return v4l_queue_g_fd(this, index, plane);
  }
  ...
  int free_bufs(unsigned from = 0) { return v4l_queue_free_bufs(this, from); }
  ...
  bool has_expbuf(cv4l_fd *fd) { return v4l_queue_has_expbuf(fd->g_v4l_fd()); }
  int export_bufs(cv4l_fd *fd, unsigned exp_type) {
    return v4l_queue_export_bufs(fd->g_v4l_fd(), this, exp_type);
  }
  void close_exported_fds() { v4l_queue_close_exported_fds(this); }
  void free(cv4l_fd *fd) { v4l_queue_free(fd->g_v4l_fd(), this); }
...
class cv4l_buffer : public v4l_buffer {
 public:
  ...
  cv4l_buffer(const cv4l_queue &q, unsigned index = 0) { init(q, index); }
  cv4l_buffer(const cv4l_buffer &b) { init(b); }
  ...
  void init(const cv4l_buffer &b) {
    ...
    if (v4l_type_is_planar(g_type())) buf.m.planes = planes;
  }
  ...
  // void s_request_fd(__s32 request_fd) {
  //   v4l_buffer_s_request_fd(this, request_fd);
  // }
  ...
  void s_length(unsigned length, unsigned plane = 0) {
    return v4l_buffer_s_length(this, plane, length);
  }
  ...
  // FIXME not supported in v4l-utils 1.14.2-1 (Ubuntu 18.04)
  // __u64 g_timestamp_ns() const {
  //   return v4l2_timeval_to_ns(v4l_buffer_g_timestamp(this));
  // }

  void s_timestamp(const timeval &tv) { v4l_buffer_s_timestamp(this, &tv); }
  void s_timestamp_ts(const timespec &ts) {
    v4l_buffer_s_timestamp_ts(this, &ts);
  }
  void s_timestamp_clock() { v4l_buffer_s_timestamp_clock(this); }
  const v4l2_timecode &g_timecode() const {
    return *v4l_buffer_g_timecode(this);
  }
