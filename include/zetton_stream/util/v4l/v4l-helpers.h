 ...
 * v4l2 functions.
 *
 * Copyright 2014-2016 Cisco Systems, Inc. and/or its affiliates. All rights
 * reserved.
 *
 * git commit: fd544473800d02e90bc289434cc44e5aa8fadd0f
 */
...
static inline ssize_t v4l_wrap_write(struct v4l_fd *f, const void *buffer,
                                     size_t n) {
...
static inline void *v4l_wrap_mmap(void *start, size_t length, int prot,
                                  int flags, struct v4l_fd *f, off_t offset) {
  return f->direct ? mmap(start, length, prot, flags, f->fd, offset)
                   : v4l2_mmap(start, length, prot, flags, f->fd, offset);
}

static inline int v4l_wrap_munmap(struct v4l_fd *f, void *start,
                                  size_t length) {
  return f->direct ? munmap(start, length) : v4l2_munmap(start, length);
}

static inline bool v4l_fd_g_direct(const struct v4l_fd *f) { return f->direct; }

static inline void v4l_fd_s_direct(struct v4l_fd *f, bool direct) {
  if (!f->is_subdev && !f->is_media) f->direct = direct;
}
...
static inline bool v4l_fd_g_direct(const struct v4l_fd *f) { return true; }
...
static inline ssize_t v4l_write(struct v4l_fd *f, const void *buffer,
                                size_t n) {
...
static inline __u32 v4l_g_type(const struct v4l_fd *f) { return f->type; }

static inline void v4l_s_type(struct v4l_fd *f, __u32 type) { f->type = type; }
...
static inline __u32 v4l_g_caps(const struct v4l_fd *f) { return f->caps; }
...
static inline bool v4l_has_radio_rx(const struct v4l_fd *f) {
  return (v4l_g_caps(f) & V4L2_CAP_RADIO) && (v4l_g_caps(f) & V4L2_CAP_TUNER);
}
...
static inline __u32 v4l_determine_type(const struct v4l_fd *f) {
  ...
  if (v4l_has_vid_cap(f)) return V4L2_BUF_TYPE_VIDEO_CAPTURE;
  if (v4l_has_vid_out(f)) return V4L2_BUF_TYPE_VIDEO_OUTPUT;
  if (v4l_has_raw_vbi_cap(f)) return V4L2_BUF_TYPE_VBI_CAPTURE;
  if (v4l_has_sliced_vbi_cap(f)) return V4L2_BUF_TYPE_SLICED_VBI_CAPTURE;
  if (v4l_has_raw_vbi_out(f)) return V4L2_BUF_TYPE_VBI_OUTPUT;
  if (v4l_has_sliced_vbi_out(f)) return V4L2_BUF_TYPE_SLICED_VBI_OUTPUT;
  if (v4l_has_sdr_cap(f)) return V4L2_BUF_TYPE_SDR_CAPTURE;
  if (v4l_has_sdr_out(f)) return V4L2_BUF_TYPE_SDR_OUTPUT;
  if (v4l_has_meta_cap(f)) return V4L2_BUF_TYPE_META_CAPTURE;
#ifdef V4L2_BUF_TYPE_META_OUTPUT
...
static inline int v4l_s_fd(struct v4l_fd *f, int fd, const char *devname,
                           bool direct) {
  ...
  if (f->devname != devname) strncpy(f->devname, devname, sizeof(f->devname));
  ...
  sel.target = sel.type == V4L2_BUF_TYPE_VIDEO_CAPTURE ? V4L2_SEL_TGT_CROP
...
static inline int v4l_open(struct v4l_fd *f, const char *devname,
                           bool non_blocking) {
...
static inline int v4l_subdev_s_fd(struct v4l_fd *f, int fd,
                                  const char *devname) {
  if (f->fd >= 0) f->close(f);
...
static inline int v4l_subdev_open(struct v4l_fd *f, const char *devname,
                                  bool non_blocking) {
  int fd = f->open(f, devname, O_RDWR | (non_blocking ? O_NONBLOCK : 0));
...
static inline int v4l_media_s_fd(struct v4l_fd *f, int fd,
                                 const char *devname) {
  if (f->fd >= 0) f->close(f);
...
static inline int v4l_media_open(struct v4l_fd *f, const char *devname,
                                 bool non_blocking) {
  int fd = f->open(f, devname, O_RDWR | (non_blocking ? O_NONBLOCK : 0));
...
static inline int v4l_reopen(struct v4l_fd *f, bool non_blocking) {
  f->close(f);
  if (f->is_subdev) return v4l_subdev_open(f, f->devname, non_blocking);
  if (f->is_media) return v4l_media_open(f, f->devname, non_blocking);
...
static inline void v4l_format_s_pixelformat(struct v4l2_format *fmt,
                                            __u32 pixelformat) {
  switch (fmt->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
...
static inline unsigned v4l_format_g_first_field(const struct v4l2_format *fmt,
  ...
  if (field != V4L2_FIELD_ALTERNATE) return field;
  if (std & V4L2_STD_525_60) return V4L2_FIELD_BOTTOM;
...
static inline unsigned v4l_format_g_flds_per_frm(
  ...
  if (field == V4L2_FIELD_ALTERNATE || field == V4L2_FIELD_TOP ||
      field == V4L2_FIELD_BOTTOM)
    return 2;
  return 1;
}

static inline void v4l_format_s_frame_height(struct v4l2_format *fmt,
                                             __u32 height) {
  if (V4L2_FIELD_HAS_T_OR_B(v4l_format_g_field(fmt))) height /= 2;
...
static inline __u32 v4l_format_g_frame_height(const struct v4l2_format *fmt) {
  ...
  if (V4L2_FIELD_HAS_T_OR_B(v4l_format_g_field(fmt))) return height * 2;
  return height;
}
...
static inline unsigned v4l_format_g_colorspace(const struct v4l2_format *fmt) {
  switch (fmt->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
...
static inline unsigned v4l_format_g_xfer_func(const struct v4l2_format *fmt) {
  switch (fmt->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
...
static inline unsigned v4l_format_g_ycbcr_enc(const struct v4l2_format *fmt) {
  switch (fmt->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
...
static inline unsigned v4l_format_g_hsv_enc(const struct v4l2_format *fmt) {
  unsigned hsv_enc = v4l_format_g_ycbcr_enc(fmt);
  if (hsv_enc == V4L2_HSV_ENC_180) return V4L2_HSV_ENC_180;
...
static inline unsigned v4l_format_g_quantization(
    const struct v4l2_format *fmt) {
  switch (fmt->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
...
static inline void v4l_format_s_flags(struct v4l2_format *fmt, unsigned flags) {
  switch (fmt->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
...
static inline unsigned v4l_format_g_flags(const struct v4l2_format *fmt) {
  switch (fmt->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
...
static inline void v4l_format_s_num_planes(struct v4l2_format *fmt,
                                           __u8 num_planes) {
  switch (fmt->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE:
...
static inline __u8 v4l_format_g_num_planes(const struct v4l2_format *fmt) {
  switch (fmt->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE:
...
static inline void v4l_format_s_bytesperline(struct v4l2_format *fmt,
  ...
  switch (fmt->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
      if (plane == 0) fmt->fmt.pix.bytesperline = bytesperline;
      break;
    case V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE:
    ...
    case V4L2_BUF_TYPE_VBI_OUTPUT:
      ...
      if (plane == 0) fmt->fmt.vbi.samples_per_line = bytesperline;
      break;
  }
}

static inline __u32 v4l_format_g_bytesperline(const struct v4l2_format *fmt,
                                              unsigned plane) {
  switch (fmt->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
...
static inline void v4l_format_s_sizeimage(struct v4l2_format *fmt,
  ...
  switch (fmt->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
      if (plane == 0) fmt->fmt.pix.sizeimage = sizeimage;
      break;
    case V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE:
...
static inline __u32 v4l_format_g_sizeimage(const struct v4l2_format *fmt,
                                           unsigned plane) {
  switch (fmt->type) {
    case V4L2_BUF_TYPE_VIDEO_CAPTURE:
    case V4L2_BUF_TYPE_VIDEO_OUTPUT:
...
static inline int v4l_g_fmt(struct v4l_fd *f, struct v4l2_format *fmt,
                            unsigned type) {
...
static inline void v4l_buffer_init(struct v4l_buffer *buf, unsigned type,
                                   unsigned memory, unsigned index) {
...
static inline bool v4l_type_is_raw_vbi(unsigned type) {
  return type == V4L2_BUF_TYPE_VBI_CAPTURE || type == V4L2_BUF_TYPE_VBI_OUTPUT;
}
...
static inline bool v4l_type_is_sdr(unsigned type) {
  return type == V4L2_BUF_TYPE_SDR_CAPTURE || type == V4L2_BUF_TYPE_SDR_OUTPUT;
}
...
static inline unsigned v4l_type_invert(unsigned type) {
  if (v4l_type_is_planar(type))
    return v4l_type_is_output(type) ? V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE
                                    : V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE;
  return v4l_type_is_output(type) ? V4L2_BUF_TYPE_VIDEO_CAPTURE
                                  : V4L2_BUF_TYPE_VIDEO_OUTPUT;
}

static inline unsigned v4l_buffer_g_num_planes(const struct v4l_buffer *buf) {
  if (v4l_type_is_planar(buf->buf.type)) return buf->buf.length;
  return 1;
}
...
static inline void v4l_buffer_s_request_fd(struct v4l_buffer *buf,
                                           __s32 request_fd) {
...
static inline const struct timeval *v4l_buffer_g_timestamp(
    const struct v4l_buffer *buf) {
  return &buf->buf.timestamp;
}

static inline void v4l_buffer_s_timestamp(struct v4l_buffer *buf,
                                          const struct timeval *tv) {
  buf->buf.timestamp = *tv;
}

static inline void v4l_buffer_s_timestamp_ts(struct v4l_buffer *buf,
...
static inline const struct v4l2_timecode *v4l_buffer_g_timecode(
    const struct v4l_buffer *buf) {
  return &buf->buf.timecode;
}

static inline void v4l_buffer_s_timecode(struct v4l_buffer *buf,
                                         const struct v4l2_timecode *tc) {
  buf->buf.timecode = *tc;
}
...
static inline void v4l_buffer_s_timestamp_src(struct v4l_buffer *buf,
                                              __u32 src) {
  buf->buf.flags &= ~V4L2_BUF_FLAG_TSTAMP_SRC_MASK;
  buf->buf.flags |= src & V4L2_BUF_FLAG_TSTAMP_SRC_MASK;
}

static inline unsigned v4l_buffer_g_length(const struct v4l_buffer *buf,
                                           unsigned plane) {
  if (v4l_type_is_planar(buf->buf.type)) return buf->planes[plane].length;
  return plane ? 0 : buf->buf.length;
}

static inline void v4l_buffer_s_length(struct v4l_buffer *buf, unsigned plane,
                                       unsigned length) {
...
static inline unsigned v4l_buffer_g_bytesused(const struct v4l_buffer *buf,
                                              unsigned plane) {
  if (v4l_type_is_planar(buf->buf.type)) return buf->planes[plane].bytesused;
  return plane ? 0 : buf->buf.bytesused;
}

static inline void v4l_buffer_s_bytesused(struct v4l_buffer *buf,
                                          unsigned plane, __u32 bytesused) {
  if (v4l_type_is_planar(buf->buf.type))
...
static inline unsigned v4l_buffer_g_data_offset(const struct v4l_buffer *buf,
                                                unsigned plane) {
  if (v4l_type_is_planar(buf->buf.type)) return buf->planes[plane].data_offset;
  return 0;
}

static inline void v4l_buffer_s_data_offset(struct v4l_buffer *buf,
                                            unsigned plane, __u32 data_offset) {
  if (v4l_type_is_planar(buf->buf.type))
    buf->planes[plane].data_offset = data_offset;
}

static inline __u32 v4l_buffer_g_mem_offset(const struct v4l_buffer *buf,
                                            unsigned plane) {
  if (v4l_type_is_planar(buf->buf.type)) return buf->planes[plane].m.mem_offset;
  return plane ? 0 : buf->buf.m.offset;
}

static inline void v4l_buffer_s_userptr(struct v4l_buffer *buf, unsigned plane,
                                        void *userptr) {
  if (v4l_type_is_planar(buf->buf.type))
...
static inline void *v4l_buffer_g_userptr(const struct v4l_buffer *buf,
                                         unsigned plane) {
  if (v4l_type_is_planar(buf->buf.type))
...
static inline void v4l_buffer_s_fd(struct v4l_buffer *buf, unsigned plane,
                                   int fd) {
  if (v4l_type_is_planar(buf->buf.type))
...
static inline int v4l_buffer_g_fd(const struct v4l_buffer *buf,
                                  unsigned plane) {
  if (v4l_type_is_planar(buf->buf.type)) return buf->planes[plane].m.fd;
  return plane ? -1 : buf->buf.m.fd;
}

static inline int v4l_buffer_prepare_buf(struct v4l_fd *f,
...
static inline int v4l_buffer_querybuf(struct v4l_fd *f, struct v4l_buffer *buf,
                                      unsigned index) {
...
static inline void v4l_queue_init(struct v4l_queue *q, unsigned type,
                                  unsigned memory) {
  unsigned i, p;
  ...
  for (i = 0; i < VIDEO_MAX_FRAME; i++)
    for (p = 0; p < VIDEO_MAX_PLANES; p++) q->fds[i][p] = -1;
}
...
static inline __u32 v4l_queue_g_length(const struct v4l_queue *q,
                                       unsigned plane) {
  return q->lengths[plane];
}

static inline __u32 v4l_queue_g_mem_offset(const struct v4l_queue *q,
                                           unsigned index, unsigned plane) {
...
static inline void *v4l_queue_g_mmapping(const struct v4l_queue *q,
                                         unsigned index, unsigned plane) {
...
static inline void *v4l_queue_g_userptr(const struct v4l_queue *q,
                                        unsigned index, unsigned plane) {
...
static inline void *v4l_queue_g_dataptr(const struct v4l_queue *q,
                                        unsigned index, unsigned plane) {
...
static inline int v4l_queue_querybufs(struct v4l_fd *f, struct v4l_queue *q,
                                      unsigned from) {
  unsigned b, p;
  int ret;
...
static inline int v4l_queue_reqbufs(struct v4l_fd *f, struct v4l_queue *q,
                                    unsigned count, unsigned int flags = 0) {
  ...
  // FIXME not supported in v4l-utils 1.14.2-1 (Ubuntu 18.04)
  // reqbufs.flags = flags;
  /*
...
static inline bool v4l_queue_has_create_bufs(struct v4l_fd *f,
                                             const struct v4l_queue *q) {
...
static inline int v4l_queue_create_bufs(struct v4l_fd *f, struct v4l_queue *q,
                                        unsigned count,
                                        const struct v4l2_format *fmt,
...
static inline int v4l_queue_mmap_bufs(struct v4l_fd *f, struct v4l_queue *q,
                                      unsigned from) {
  unsigned b, p;
  ...
  for (b = from; b < v4l_queue_g_buffers(q); b++) {
    for (p = 0; p < v4l_queue_g_num_planes(q); p++) {
      ...
      if (q->memory == V4L2_MEMORY_MMAP)
        m = v4l_mmap(f, v4l_queue_g_length(q, p),
                     v4l_queue_g_mem_offset(q, b, p));
      else if (q->memory == V4L2_MEMORY_DMABUF)
        m = mmap(NULL, v4l_queue_g_length(q, p), PROT_READ | PROT_WRITE,
                 MAP_SHARED, v4l_queue_g_fd(q, b, p), 0);
...
static inline int v4l_queue_alloc_bufs(struct v4l_fd *f, struct v4l_queue *q,
                                       unsigned from) {
  unsigned b, p;
...
static inline int v4l_queue_obtain_bufs(struct v4l_fd *f, struct v4l_queue *q,
                                        unsigned from) {
  if (q->memory == V4L2_MEMORY_USERPTR) return v4l_queue_alloc_bufs(f, q, from);
  return v4l_queue_mmap_bufs(f, q, from);
}

static inline int v4l_queue_release_bufs(struct v4l_fd *f, struct v4l_queue *q,
                                         unsigned from) {
  if (q->memory == V4L2_MEMORY_USERPTR) return v4l_queue_free_bufs(q, from);
...
static inline void v4l_queue_close_exported_fds(struct v4l_queue *q) {
  ...
  if (q->memory != V4L2_MEMORY_MMAP) return;
...
static inline void v4l_queue_buffer_init(const struct v4l_queue *q,
                                         struct v4l_buffer *buf,
                                         unsigned index) {
...
static inline int v4l_query_ext_ctrl(v4l_fd *f, struct v4l2_query_ext_ctrl *qec,
  ...
  if (next_compound && !f->have_query_ext_ctrl) {
    if (!next_ctrl) return -EINVAL;
    next_compound = false;
  }
  if (next_compound) qec->id |= V4L2_CTRL_FLAG_NEXT_COMPOUND;
  if (next_ctrl) {
    if (f->have_next_ctrl)
  ...
  if (f->have_query_ext_ctrl) return v4l_ioctl(f, VIDIOC_QUERY_EXT_CTRL, qec);
  ...
  for (;;) {
    ...
    if (!next_ctrl || f->have_next_ctrl) return ret;
    if (qec->id >= V4L2_CID_PRIVATE_BASE) return ret;
    qec->id++;
  }
  qec->id = qc.id;
  qec->type = qc.type;
...
static inline int v4l_g_ext_ctrls(v4l_fd *f, struct v4l2_ext_controls *ec) {
  ...
  if (f->have_ext_ctrls) return v4l_ioctl(f, VIDIOC_G_EXT_CTRLS, ec);
...
static inline int v4l_s_ext_ctrls(v4l_fd *f, struct v4l2_ext_controls *ec) {
  ...
  if (f->have_ext_ctrls) return v4l_ioctl(f, VIDIOC_S_EXT_CTRLS, ec);
  if (ec->count == 0) return 0;
  for (i = 0; i < ec->count; i++) {
...
static inline int v4l_try_ext_ctrls(v4l_fd *f, struct v4l2_ext_controls *ec) {
  ...
  if (f->have_ext_ctrls) return v4l_ioctl(f, VIDIOC_TRY_EXT_CTRLS, ec);
  if (ec->count == 0) return 0;
  for (i = 0; i < ec->count; i++) {
    struct v4l2_queryctrl qc;
    int ret;
...
static inline int v4l_g_selection(v4l_fd *f, struct v4l2_selection *sel) {
  ...
  if (f->have_selection) return v4l_ioctl(f, VIDIOC_G_SELECTION, sel);
  crop.type = sel->type;
  cc.type = sel->type;
...
static inline int v4l_s_selection(v4l_fd *f, struct v4l2_selection *sel) {
  ...
  if (f->have_selection) return v4l_ioctl(f, VIDIOC_S_SELECTION, sel);
  crop.type = sel->type;
  ret = v4l_ioctl(f, VIDIOC_G_CROP, &crop);
  if (ret) return ret;
  if (sel->type == V4L2_BUF_TYPE_VIDEO_CAPTURE &&
...
static inline void v4l_frame_selection(struct v4l2_selection *sel,
                                       bool to_frame) {
  switch (sel->target) {
    case V4L2_SEL_TGT_CROP:
    case V4L2_SEL_TGT_CROP_DEFAULT:
    case V4L2_SEL_TGT_CROP_BOUNDS:
#ifdef V4L2_TYPE_IS_CAPTURE
      if (V4L2_TYPE_IS_CAPTURE(sel->type)) return;
#else
      if (v4l_type_is_capture(sel->type)) return;
#endif
      break;
    case V4L2_SEL_TGT_COMPOSE:
    case V4L2_SEL_TGT_COMPOSE_DEFAULT:
    ...
    case V4L2_SEL_TGT_COMPOSE_PADDED:
      if (V4L2_TYPE_IS_OUTPUT(sel->type)) return;
      break;
    default:
      return;
  }
...
static inline int v4l_g_frame_selection(v4l_fd *f, struct v4l2_selection *sel,
                                        __u32 field) {
  int ret = v4l_g_selection(f, sel);

  if (V4L2_FIELD_HAS_T_OR_B(field)) v4l_frame_selection(sel, true);
  return ret;
}

static inline int v4l_s_frame_selection(v4l_fd *f, struct v4l2_selection *sel,
                                        __u32 field) {
  int ret;

  if (V4L2_FIELD_HAS_T_OR_B(field)) v4l_frame_selection(sel, false);
