...
CV4l2StreamSource::CV4l2StreamSource()
    : fd_(), buffers_(nullptr), n_buffers_(4), is_capturing_(false) {}

CV4l2StreamSource::~CV4l2StreamSource() { Shutdown(); }

bool CV4l2StreamSource::Init(const StreamOptions& options) {
  options_ = options;
  monochrome_ = false;
...
bool CV4l2StreamSource::WaitForDevice() {
  if (is_capturing_) {
    ADEBUG << "is capturing";
...
bool CV4l2StreamSource::Capture(const CameraImagePtr& raw_image) {
  // 0. prepare
...
bool CV4l2StreamSource::IsCaptuering() { return is_capturing_; }

void CV4l2StreamSource::Shutdown() {
  StopCapturing();
  UninitDevice();
  CloseDevice();
}

bool CV4l2StreamSource::OpenDevice() {
...
bool CV4l2StreamSource::CloseDevice() {
  if (fd_.close() != 0) {
...
bool CV4l2StreamSource::InitDevice() {
...
bool CV4l2StreamSource::UninitDevice() {
  switch (options_.io_method) {
    case StreamIoMethod::IO_METHOD_MMAP:
...
bool CV4l2StreamSource::StartCapturing() {
...
bool CV4l2StreamSource::StopCapturing() {
...
bool CV4l2StreamSource::ReadFrame(CameraImagePtr raw_image) {
...
bool CV4l2StreamSource::ProcessImage(
    std::array<void*, VIDEO_MAX_PLANES> mplane_data,
    std::array<unsigned int, VIDEO_MAX_PLANES> mplane_size,
    CameraImagePtr dest) {
